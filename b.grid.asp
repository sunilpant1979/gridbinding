...
function BGrid_Display() {
	...
	if(Request.Form("action").Item=="add" || (this._error && Request.Form("action").Item=="savenew")) {
		...
		this._rs.CancelUpdate();
	} else if(Request.Form("action").Count==1 && (Request.Form("action").Item=="edit" || (Request.Form("action").Item=="save" && this._error))) {
		...
		if(this._rs.EOF || this._rs.BOF || this._rs.RecordCount==0) {
			...
			while(!this._rs.EOF) {
				...
				if(this._pk!="") {
					...
					Out(" <a class=\"smallbutton\" href=\"javascript:document.forms[0].action.value='edit';"+sCode+"document.forms[0].submit()\">Edit</a>");
					Out(" <a class=\"smallbutton\" href=\"javascript:if(confirm('Delete this record?')) { document.forms[0].action.value='del';"+sCode+"document.forms[0].submit();}\">Del</a>");
				}
				Out("</nobr></td>");
		...
		if(nVisibleColumns) {
			Out("<td>&nbsp;</td>");
			Out("<td colspan=\"" + nVisibleColumns + "\">");
			Out("<a class=\"button\" href=\"javascript:document.forms[0].action.value='prev';document.forms[0].submit()\">Previous</a> ");
			Out("<a class=\"button\" href=\"javascript:document.forms[0].action.value='next';document.forms[0].submit()\">Next</a> ");
			Out("<a class=\"button\" href=\"javascript:document.forms[0].action.value='first';document.forms[0].submit()\">First</a> ");
			Out("<a class=\"button\" href=\"javascript:document.forms[0].action.value='last';document.forms[0].submit()\">Last</a> ");
			if(this._pk!="")
				Out("<a class=\"button\" href=\"javascript:document.forms[0].action.value='add';document.forms[0].submit()\">Add</a> ");
			Out("Page " + (this._pageNo -(-1)) + " of " + Math.ceil(this._rs.RecordCount / this._pageSize) + " ");
		} else {
			Out("<td>");
			if(this._pk!="")
				Out("<a class=\"button\" href=\"javascript:document.forms[0].action.value='add';document.forms[0].submit()\">Add</a>");
